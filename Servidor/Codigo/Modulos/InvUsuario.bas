...
If UserList(UserIndex).Invent.Object(SLOT).Amount <= 0 Then
    ...
    UserList(UserIndex).Invent.Object(SLOT).ObjIndex = 0 'ULISES: Pone a Cero el Objeto de inventario
    UserList(UserIndex).Invent.Object(SLOT).Amount = 0 'ULISES: Pone a Cero la Cantidad de Objetos
End If
...
If (Obj.ObjIndex > 964) Then Obj.ObjIndex = Obj.ObjIndex - 1

Call SendData(sndRoute, sndIndex, sndMap, "HO" & ObjData(Obj.ObjIndex).GrhIndex & "," & X & "," & Y)

'UlISES:Hay que diseñar limites para Obj.ObjIndex
End Sub
...
Exit Sub
errhandler:
Call LogError("ULISES: Alguien Jodio un SLOT al EquiparInvItem Slot:" & SLOT)
End Sub
