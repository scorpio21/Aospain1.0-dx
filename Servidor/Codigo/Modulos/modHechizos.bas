...
'ULISES CONTROL PARA SABER SI TENEMOS YA EL HECHIZO
...
'ULISES: AQUI SE CONTROLA LOS HECHIZOS KE SE AÑADEN
'Ejem. OBJ.DAT Olocausto HECHIZOINDEX = 2
'[OBJ199]
'Name = Olocausto
'GrhIndex = 609
'Valor = 356
...
If Not TieneHechizo(hIndex, UserIndex) Then
    ...
    If UserList(UserIndex).Stats.UserHechizos(j) <> 0 Then
        Call SendData(ToIndex, UserIndex, 0, "||No tienes espacio para mas hechizos." & FONTTYPE_INFO)
    Else
    ...
    End If
Else
    Call SendData(ToIndex, UserIndex, 0, "||Ya tienes ese hechizo." & FONTTYPE_INFO)
End If
...
'ULISES: SELECCIONA EL HECHIZO Y LO LANZA
...
ElseIf Hechizos(hIndex).SubeHP = 2 Then
    ...
    If Npclist(NpcIndex).Attackable = 0 Then
        Call SendData(ToIndex, UserIndex, 0, "||No puedes atacar a ese npc." & FONTTYPE_INFO)
        Exit Sub
    End If
...
ElseIf Hechizos(H).SubeHP = 2 Then
    ...
    If UserIndex = tempChr Then
        Call SendData(ToIndex, UserIndex, 0, "||No puedes atacarte a vos mismo." & FONTTYPE_FIGHT)
        Exit Sub
    End If
