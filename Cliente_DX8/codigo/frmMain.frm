...
' [DX8] gDSB/gD/gW/dsE/Pos/endEvent/DirectXEvent eliminados.
' El sonido se maneja via DSBuffers() en DX_InIt.bas y Mod_Wav.bas con DirectSound8.

Private Sub CreateEvent()
    ' [DX8] No necesario - events de DX7 eliminados
End Sub

Private Function LoadSoundBufferFromFile(sFile As String) As Integer
    ' [DX8] Delegado a PlayWave en Mod_Wav.bas
    On Error Resume Next
    Call PlayWaveDS(sFile)
End Function

Public Sub Play(ByVal nombre As String, Optional ByVal LoopSound As Boolean = False)
    If Fx = 1 Then Exit Sub
    On Error Resume Next
    Call PlayWaveDS(nombre)
End Sub
...
If UserParalizado = True Or UserCiego = True Or UserEstupido = True Then     '65~190~156~0~0
    ...
    Exit Sub
Else
    ' [DX8] endEvent/DirectX.DestroyEvent eliminados
    If prgRun = True Then
        prgRun = False
        Cancel = 1
    End If
...
Public Sub StopSound()
    ' [DX8] Audio maneja los buffers internamente via clsAudio
    On Error Resume Next
    Call Audio.StopWave
End Sub
