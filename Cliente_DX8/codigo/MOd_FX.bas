...
For i = 1 To CantidadFX
    If FXs(i).vida > 0 Then
         ' [DX8] FX via engine
         Dim dxLightFX(0 To 3) As Long
         dxLightFX(0) = &HFFFFFFFF : dxLightFX(1) = &HFFFFFFFF
         dxLightFX(2) = &HFFFFFFFF : dxLightFX(3) = &HFFFFFFFF
         Dim fxGrhIdx As Long
         fxGrhIdx = GrhData(Grh(FXs(i).GrhIndex).GrhIndex).Frames(Grh(FXs(i).GrhIndex).FrameCounter)
         Call engine.Device_Box_Textured_Render(fxGrhIdx, FXs(i).x, FXs(i).y, GrhData(fxGrhIdx).pixelWidth, GrhData(fxGrhIdx).pixelHeight, dxLightFX(), GrhData(fxGrhIdx).sX, GrhData(fxGrhIdx).sY, True)
    End If
Next
End Sub
